//! 入瓶图片 —— 复用顶层媒体服务([`crate::media`]):把投放消息里的图登记 + 排队下载,
//! **阻塞等**每张落盘完成,再读回字节喂内容审核([`crate::moderation`])。
//!
...
pub struct StoredImage {
    /// 内容 md5(捞取时经 [`crate::media::resolve`] 取路径重发)。
    pub md5: String,
    /// 原始图片字节,供调用方喂给图片审核。
...
pub async fn fetch_and_store(content: &[Segment]) -> Vec<StoredImage> {
    let refs = crate::media::scan(content);
    if refs.is_empty() {
        return Vec::new();
    }
    let tickets = crate::media::ingest(refs).await;
    let mut out = Vec::with_capacity(tickets.len());
    for ticket in tickets {
        let stored = match crate::media::wait(&ticket, WAIT_TIMEOUT).await {
            Ok(s) => s,
            Err(e) => {
